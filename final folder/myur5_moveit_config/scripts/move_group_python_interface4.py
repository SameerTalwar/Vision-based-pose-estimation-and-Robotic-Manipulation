...
class MoveItContext(object):
    ...
    def Mareacallback(self,data):
        ...
        if (abs(self.erry)+abs(self.errz)+abs(1-(Marea/90000.0))>1)&(self.atHome):
            ...
            pose_goal.position.x = current_pose.position.x + 0.05*(1-(Marea/90000.0))
            pose_goal.position.y = current_pose.position.y + 0.0005*self.erry
            pose_goal.position.z = current_pose.position.z + 0.0005*self.errz
